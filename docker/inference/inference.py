...
# Import our new Data & UI Manager
import data_manager
...
def parse_args():
    ...
    parser.add_argument("--par-model", type=str, default="baseline_v4_prod.onnx")
...
class PedestrianState:
    ...
    def update(self, q_score, probs, schema_keys, crop_bgr):
        ...
        # Handle view switching (person turns around)
        ...
        # Buffer collection phase
        if self.is_collecting:
            # Add the crop_bgr to the buffer so we can retrieve the best one later
            self.buffer.append((q_score, probs, crop_bgr.copy()))
            
            # Sort by q_score (bounding box size) descending
            self.buffer.sort(key=lambda x: x[0], reverse=True)
            if len(self.buffer) > self.top_k: self.buffer = self.buffer[:self.top_k]
            
            # Lock triggered - Buffer is full
            if len(self.buffer) == self.top_k:
                # 1. Calculate the stable consensus probabilities across all 10 frames
                agg_probs = np.mean([p for _, p, _ in self.buffer], axis=0)
                self.locked_attrs = data_manager.parse_attributes(agg_probs, schema_keys)
                
                # 2. Determine the true view based on the consensus
                self.active_view = views[np.argmax(agg_probs[0:3])]
                
                # 3. Save the best crop ONLY now that we are sure of the view
                # (Since buffer is sorted, index 0 is guaranteed to be the highest q_score)
                ...
                if self.active_view not in self.best_crops or best_q > self.best_crops[self.active_view]['score']:
                    self.best_crops[self.active_view] = { 
                        'score': best_q, 
                        'crop': best_crop, 
                        'probs': agg_probs  # Save the stable aggregated scores, not single-frame
                    }
        ...
        # Fallback for UI drawing while still collecting
